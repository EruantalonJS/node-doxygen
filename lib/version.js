...
function downloadVersion(version, protocol, hostName, fileRoute) {
    ...
    var dataPromise = getDataPromise(version, protocol, hostName, fileRoute);
 ...
 * @param {String} fileRoute - The route from which to download the doxygen file
 */
function getDataPromise(version, protocol, hostName, fileRoute) {
    if (protocol == "http") {
        return httpDownload(hostName, fileRoute);
...
function getFileRoute(version) {
    ...
    return constants.default.downloadFileRoute
        .replace("%doxygenName%", osInfo.doxygenName)
...
function unCompressFiles(buffer, destinationPath, isDmg) {
    if (isDmg) {
        return decompressDmg(buffer, destinationPath);
    }
    else {
