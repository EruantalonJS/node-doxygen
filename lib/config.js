...
function createConfig(doxygenOptions, configPath) {
    ...
    if (doxygenOptions){
        if (doxygenOptions["INPUT"]){
            if(Array.isArray(doxygenOptions["INPUT"])) {
                doxygenOptions["INPUT"] = doxygenOptions["INPUT"]
                                                .map(relativePath => stringAsPath(relativePath));
            }
            else {
                doxygenOptions["INPUT"] = stringAsPath(doxygenOptions["INPUT"]);
            }
        } else if (doxygenOptions["OUTPUT_DIRECTORY"]){
            doxygenOptions["OUTPUT_DIRECTORY"] = stringAsPath(doxygenOptions["OUTPUT_DIRECTORY"]);
        }
    }
 ...
 * @param {String} relativePath - The string to be stored as a path
 */
function stringAsPath(relativePath){
